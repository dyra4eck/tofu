...
ВМ клонируются из подготовленных cloud-init шаблонов (RedOS 8, Astra Linux 1.7 и Astra Linux 1.8) и настраиваются через cloud-init.
...
```
GitLab CI -> tofu clone -> VM из шаблона 351 / 352 / 353
                 |
...
Чтобы создать стенд из Astra 1.7, передайте `STAND_OS=astra17`; для Astra 1.8 используется `STAND_OS=astra`.
...
| `STAND_OS`     | Какой шаблон клонировать: `astra17`, `astra` (1.8) или `redos`                         | `astra`           |
| `STAND_CORES`  | Количество ядер                                                                        | `6`               |
| `STAND_MEMORY` | Память **в гигабайтах** (в tfvars уйдёт ×1024), не больше 50                           | `16`              |
...
| `ASTRA_TEMPLATE_ID`       | VMID шаблона Astra 1.8 (сейчас `352`)                    |
| `ASTRA17_TEMPLATE_ID`     | VMID шаблона Astra 1.7 (по умолчанию `353`)              |
...
| `353` | Astra Linux 1.7             | 30G                      |

Размер диска шаблона — это **минимум** для любого клона. Для VMID `353` сначала проверьте фактический размер диска в Proxmox. Задать `STAND_DISK`
...
### Состав шаблонов Astra 1.7 и 1.8

Для обоих вариантов pipeline выбирает Astra vendor-data для регистрации DNS. Установленный состав и cloud-init исправления должны соответствовать конкретному образу; перед клонированием Astra 1.7 проверьте совместимость с `local:snippets/dns-register-astra.yaml` и имя интерфейса `eth0` после generalize.

Для Astra 1.8 установлено: `cloud-init`, `cloud-guest-utils`, `qemu-guest-agent`, `chrony`, консольные утилиты, SSH-сервер. Без графики, без ufw, без средств виртуализации.
...
7. `qm template <VMID>` и обновить соответствующую CI/CD переменную: `ASTRA17_TEMPLATE_ID`, `ASTRA_TEMPLATE_ID` или `REDOS_TEMPLATE_ID`.
